...
namespace AuthenticationSdk.core
{
    ...
    *============================================================================================*/
    public class MerchantConfig
    {
        ...
        /// When "SHARED_SECRET", MerchantKeyId and MerchantSecretKey are used for JWT token signing instead of P12 certificate fields.
        /// </summary>
        public string JwtKeyType { get; set; }

        /// <summary>
        /// Returns true if JwtKeyType is "SHARED_SECRET" (case-insensitive).
        /// </summary>
        public bool IsSharedSecretKeyType => string.Equals(JwtKeyType, Constants.JwtKeyTypeSharedSecret, StringComparison.OrdinalIgnoreCase);
        ...
        private void SetValuesFromAppConfig(NameValueCollection merchantConfigSection, Dictionary<string, string> mapToControlMLEonAPI)
        {
            ...
            JwtKeyType = merchantConfigSection["jwtKeyType"];
            if (string.IsNullOrEmpty(JwtKeyType))
            {
                JwtKeyType = Constants.JwtKeyTypeP12;
            }
            ...
            // Parse isSDK flag - trim whitespace and ignore case
            var isSDKAppConfig = merchantConfigSection["isSDK"];
            IsSDK = string.Equals(isSDKAppConfig?.Trim(), "true", StringComparison.OrdinalIgnoreCase);
        }
            ...
            try
            {
                if (merchantConfigDictionary != null)
                {
                    ...
                    if (Equals(authTypeInput, Enumerations.AuthenticationType.JWT))
                    {
                        ...
                        // Read jwtKeyType (defaults to P12)
                        if (merchantConfigDictionary.ContainsKey("jwtKeyType"))
                        {
                            JwtKeyType = merchantConfigDictionary["jwtKeyType"];
                        }
                        if (string.IsNullOrEmpty(JwtKeyType))
                        {
                            JwtKeyType = Constants.JwtKeyTypeP12;
                        ...
                        if (IsSharedSecretKeyType)
                        {
                            // JWT with SHARED_SECRET — read symmetric key fields
                            if (merchantConfigDictionary.ContainsKey("merchantsecretKey"))
                            {
                                MerchantSecretKey = merchantConfigDictionary["merchantsecretKey"];
                            }
                            if (merchantConfigDictionary.ContainsKey("merchantKeyId"))
                            {
                                MerchantKeyId = merchantConfigDictionary["merchantKeyId"];
                            }
                        }
                        else
                        {
                            // JWT with P12 — read certificate fields
                    ...
                    // Parse isSDK flag - trim whitespace and ignore case
                    if (merchantConfigDictionary.ContainsKey("isSDK"))
                    {
                        IsSDK = string.Equals(merchantConfigDictionary["isSDK"]?.Trim(), "true", StringComparison.OrdinalIgnoreCase);
                    }
                    else
                    {
                        IsSDK = false;
        ...
        private void ValidateProperties()
        {
            ...
            else if (IsJwtTokenAuthType)
            {
                ...
                // Validate jwtKeyType
                if (!string.Equals(JwtKeyType, Constants.JwtKeyTypeP12, StringComparison.OrdinalIgnoreCase)
                    && !string.Equals(JwtKeyType, Constants.JwtKeyTypeSharedSecret, StringComparison.OrdinalIgnoreCase))
                {
                    Logger.Error(Constants.InvalidJwtKeyTypeErrorMessage + "Provided value: " + JwtKeyType);
                    throw new Exception($"{Constants.InvalidJwtKeyTypeErrorMessage}Provided value: {JwtKeyType}");
                }
                ...
                if (IsSharedSecretKeyType)
                {
                    ...
                    if (string.IsNullOrEmpty(MerchantKeyId))
                    {
                        Logger.Error("Merchant Key ID cannot be null or empty for JWT with SHARED_SECRET");
                        throw new Exception($"{Constants.ErrorPrefix} Merchant Config field - MerchantKeyId is Mandatory when jwtKeyType is SHARED_SECRET");
                    }

                    if (string.IsNullOrEmpty(MerchantSecretKey))
                    {
                        Logger.Error("Merchant Secret Key cannot be null or empty for JWT with SHARED_SECRET");
                        throw new Exception($"{Constants.ErrorPrefix} Merchant Config field - MerchantSecretKey is Mandatory when jwtKeyType is SHARED_SECRET");
                    }
                }
                else
                {
